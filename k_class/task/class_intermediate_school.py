# 학교
# 이름, 지역, 학생 수, 선생님 수 - 전부 학교가 가지고 있음
# 학교 정보 출력 메소드

# 선생님
# 이름, 과목, 학교
# 선생님이 추가될 때마다 선생님 수 1증가 - '선생님' 객체가 선언될 때마다 +1
# 준비된 학생들에게 해당 과목을 가르치면 학생들의 능력치 1증가
# 선생님 정보 출력 메소드

# 학생
# 이름, 학년(grade), 학교, 능력치(초기값: 0), 담임 선생님
# 학생이 추가될 때마다, 학생 수 1증가 - '학생' 객체가 선언될 때마다 +1
# 학생 정보 출력 메소드

# 학교가 학생과 선생님의 정보를 포함 - 학생, 선생님 클래스 불러와서 사용
# 학생 - 학생이 선생님의 데이터(근무지, 이름)를 가지고 있어야 된다?
# 만약 그렇다면, 만들어진 선생님 클래스에서 해당 항목만 불러와서 사용

# 학교 클래스 - 이름, 지역, 학생 수, 선생님 수
# 학생, 선생님 수는 각각의 클래스에서 가져와서 사용
class School:
